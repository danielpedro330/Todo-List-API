...
// node_modules/vitest/dist/chunks/vi.Bgcdy3bQ.js
var unsupported = [
  "matchSnapshot",
  "toMatchSnapshot",
...
// src/use-cases/search-task.spec.ts
var taskRepository;
var sut;
describe("Search Task Use Case", () => {
  ...
  it("Should be able to search task", async () => {
    ...
    globalExpect(task).toHaveLength(1);
    globalExpect(task).toEqual([globalExpect.objectContaining({ title: "Estudar JavaScript" })]);
  ...
  it("Should be able to pagened search task", async () => {
    ...
    globalExpect(task).toHaveLength(2);
    globalExpect(task).toEqual([
      globalExpect.objectContaining({ title: "Estudar TypeScript 21" }),
      globalExpect.objectContaining({ title: "Estudar TypeScript 22" })
    ]);
  });
});
